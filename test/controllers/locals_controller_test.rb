...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { address: @local.address, city: @local.city, state: @local.state, zip: @local.zip }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { address: @local.address, city: @local.city, state: @local.state, zip: @local.zip }
    assert_redirected_to local_path(assigns(:local))
  end
